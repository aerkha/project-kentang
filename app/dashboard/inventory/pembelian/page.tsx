...
export default function PembelianPage() {
  ...
  // Komponen Nota (Dipanggil 2x: Satu untuk layar, Satu untuk printer)
  const renderNota = (isPrintMode = false) => {
    if (!printData) return null;
    return (
      <div className={`bg-white text-black font-sans ${isPrintMode ? 'p-0' : 'p-6 sm:p-8 border rounded-lg shadow-sm'}`}>
  ...
  return (
    <>
      {/* ========================================= */}
      {/* 1. KONTEN HALAMAN (Sembunyi saat print)   */}
      {/* ========================================= */}
      <div className="space-y-6 print:hidden">
        ...
        <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) setFileTf(null); }}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            {/* ... Isi form tidak berubah (saya sembunyikan untuk memperpendek layar saat Anda membaca ini, tapi di kodenya tetap lengkap di bawah) ... */}
        ...
        {/* Modal Pratinjau Nota (Untuk dilihat di layar) */}
      ...
      {/* ==================================================================== */}
      {/* 2. KONTEN PRINT (Tersembunyi di layar, HANYA MUNCUL DI KERTAS PRINT) */}
      {/* ==================================================================== */}
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          /* Matikan paksa pengunci scroll dari Radix Modal */
          body, html { overflow: visible !important; height: auto !important; }
          /* Sembunyikan Layout bawaan NextJS (Sidebar, Header, dll) */
          aside, header, nav { display: none !important; }
          /* Sembunyikan Pop-up Dialog Radix saat Print */
          [data-radix-portal] { display: none !important; }
          /* Sembunyikan div utama yang punya class print:hidden (sudah otomatis dari tailwind) */
        }
      `}} />

      {/* Tampilkan Nota Versi Kertas (ditaruh paling luar agar tidak kena blokir) */}
      {printData && (
