 ...
 * handler — PKS yang sudah pernah dikirim tidak dikirim ulang.
 */
...
export async function GET(req: NextRequest) {
  ...
  if (isTest) {
    ...
    return NextResponse.json({ mode: "test", adminEmail, investorEmail, waStatus });
  }
  ...
  try {
    ...
    if (dueMous.length === 0) {
      return NextResponse.json({
        sent: 0,
        message: "Tidak ada PKS yang mencapai akhir periode hari ini",
      });
    }
    ...
    if (toSend.length === 0) {
      return NextResponse.json({ sent: 0, message: "Semua reminder akhir periode hari ini sudah terkirim" });
    }
