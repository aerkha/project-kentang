 ...
 * Body: { transaksiId, keterangan, investorId, jumlah, buktiUrl, brokerName?, noPks?, modal? }
...
interface NotifyBody {
  ...
  jumlah:      number;
  buktiUrl:    string;
  // Tambahan Opsional untuk Format WA Baru
  brokerName?: string;
  noPks?:      string;
  modal?:      number;
}
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.08);">
    ...
    <div style="background:#16a34a;padding:28px 32px;">
      ...
      <p style="margin:6px 0 0;color:#bbf7d0;font-size:13px;">${tanggal} · MinBun</p>
    </div>
...
export async function POST(req: NextRequest) {
  ...
  // Kirim WA dan email ke investor secara paralel
  const [waStatus, emailStatus] = await Promise.all([
    sendWhatsApp(investorPhone, waOpts).catch((e) => {
      errors.push(`WA: ${String(e)}`);
      return "failed" as ChannelStatus;
    }),
    sendEmail(investorEmail, emailOpts).catch((e) => {
