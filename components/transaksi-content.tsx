...
interface TransaksiContextType {
  ...
  triggerAutorenewal:   (id: string) => Promise<void>; 
}
...
async function generateCustomId(isAutorenewal: boolean = false): Promise<string> {
  try {
    ...
    const max = res.reduce((m, r) => {
      ...
      const n = Number.parseInt(numStr, 10) || 0;
      return Math.max(m, n);
    }, 0);
...
export function TransaksiProvider({ children }: Readonly<{ children: ReactNode }>) {
    ...
    Promise.allSettled([
      ...
      if (invResult.status === "fulfilled") {
        for (const r of invResult.value) {
          ...
          const entries = invMap.get(tid);
          if (entries === undefined) {
            const newEntries: TransaksiInvestorEntry[] = [];
            invMap.set(tid, newEntries);
            newEntries.push(recordToInvestorEntry(r));
          } else {
            entries.push(recordToInvestorEntry(r));
          }
        }
      }
      setTransaksis(
        trxRecords.map((r) =>
          recordToTransaksi(r as Record<string, unknown>, map, invMap.get(r.id) ?? []),
        ),
      );
    });
  ...
  const syncInvestorInfo = async (
    ...
    const updateInvestorEntry = (e: TransaksiInvestorEntry) =>
      e.investorId === investorId
        ? { ...e, investorName, investorBrokerName }
        : e;

    const updateTransaksi = (t: Transaksi) => ({
      ...t,
      investorEntries: t.investorEntries.map(updateInvestorEntry),
    });

    setTransaksis((prev) => prev.map(updateTransaksi));
  };
  ...
  const incrementAlphaSuffix = (suffix: string): string => {
    let newSuffix = "";
    let carry = true;
    for (let i = suffix.length - 1; i >= 0; i--) {
      ...
      const charCode = suffix.codePointAt(i)!;
      if (charCode === 90) {
        newSuffix = "A" + newSuffix;
      } else {
        newSuffix = String.fromCodePoint(charCode + 1) + newSuffix;
        carry = false;
      }
    }
    return carry ? "A" + newSuffix : newSuffix;
  };

  const getNextCustomId = (oldId: string): string => {
    const match = /^(TRX-\d+)([A-Z]*)$/i.exec(oldId);
    if (!match) return oldId + "B";
    const [, base, suffix] = match;
    return base + (suffix ? incrementAlphaSuffix(suffix.toUpperCase()) : "B");
  };

  const buildCatatanAkhir = (existing: string | undefined, note: string) =>
    existing ? `${existing}\n${note}` : note;

  const getNextTransactionDate = (date: string, period: number) => {
    const [y, m, d] = date.slice(0, 10).split("-").map(Number);
    return new Date(Date.UTC(y, m - 1, d + period)).toISOString().slice(0, 10);
  };

  // ─── MESIN AUTORENEWAL INSTAN ───
  const triggerAutorenewal = async (oldTrxId: string) => {
    const old = transaksis.find(t => t.id === oldTrxId);
    if (!old?.isAutorenewal) return;

    const pbId = await resolvePbId(old.id);
    if (!pbId) return;
    ...
    if (isExpired) {
      await updateTransaksi(old.id, {
        isAutorenewal: false,
        catatanAkhir: buildCatatanAkhir(
          old.catatanAkhir,
    ...
    const newCustomId = getNextCustomId(old.id);
    const period = parsePeriodeDays(old.description);
    const nextDate = getNextTransactionDate(old.date, period);
    ...
    try {
      const record = await pb.collection("transaksis").create({
        ...
        status: "berjalan",
        catatanAkhir: `[Sistem] Transaksi autorenewal lanjutan dari ${old.id}`,
      });

      const entriesToCreate = old.investorEntries.map(e => ({
        transaksiId: record.id,
        mouId: e.mouId,
        investorId: e.investorId,
        investorName: e.investorName,
        investorBrokerName: e.investorBrokerName,
      ...
      await Promise.all(
        entriesToCreate.map(e => pb.collection("transaksi_investors").create(e)),
      );

      const newTrx = recordToTransaksi(record, map, old.investorEntries);
      setTransaksis((prev) => [...prev, newTrx]);

      await updateTransaksi(old.id, {
        isAutorenewal: false,
        catatanAkhir: buildCatatanAkhir(
          old.catatanAkhir,
          `[Sistem] Autorenewal otomatis sukses digandakan ke ${newCustomId}`,
        ),
      });

      toast.success(`🪄 Autorenewal dieksekusi seketika! Transaksi ${newCustomId} untuk bulan depan telah mengudara di daftar.`);
    } catch (err) {
      console.error("Gagal menjalankan autorenewal:", err);
      toast.error(`Sistem gagal menduplikasi autorenewal untuk ${old.id}`);
    }
  };
  ...
  return (
    <TransaksiContext.Provider value={providerValue}>
      {children}
